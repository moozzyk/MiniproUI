...
struct SupportedDevicesTests {
    ...
    @Test func testSupportedLogicICsHappyPath() async throws {
        ...
        let supportedDevices = try SupportedDevicesProcessor.run(miniproResult, infoicPath: infoicPath)
        #expect(
    ...
    @Test func testSupportedDeviceChecksForErrors() {
        #expect(throws: MiniproAPIError.unknownError("Error")) {
            try SupportedDevicesProcessor.run(
                InvocationResult(exitCode: 0, stdOut: Data(), stdErr: "Error"),
